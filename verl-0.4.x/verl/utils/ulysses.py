...
def ulysses_pad_and_slice_inputs_3d(
    input_ids_rmpad: torch.Tensor,
    position_ids_rmpad: Optional[torch.Tensor] = None,
    sp_size: int = 1
):
    """
    Pad and slice 3D input_ids [bsz, seqlen, extra_dim]
    and position_ids [bsz, seqlen, extra_dim] to make seqlen axis divisible by sp_size.
    """
    # 1. Pad seqlen axis with ulysses_pad (需要ulysses_pad支持3D输入，并只pad第二维)
    input_ids_rmpad, position_ids_rmpad, pad_size = ulysses_pad(
        input_ids_rmpad, position_ids_rmpad, sp_size
    )
    # 2. Slice seqlen axis（dim=1）
    input_ids_rmpad = slice_input_tensor(input_ids_rmpad, dim=1, padding=False)
    if position_ids_rmpad is not None:
        position_ids_rmpad = slice_input_tensor(position_ids_rmpad, dim=1, padding=False)
    return input_ids_rmpad, position_ids_rmpad, pad_size
